...
def lsi_recommendation():
    vectorizer = TfidfVectorizer()
    tfidf_matrix = vectorizer.fit_transform(df["features"])
    svd = TruncatedSVD(n_components=5)
    lsi_matrix = svd.fit_transform(tfidf_matrix)
    test_lsi = svd.transform(vectorizer.transform([test_features_str]))
    similarity_scores = cosine_similarity(test_lsi, lsi_matrix).flatten()
    top_indices = similarity_scores.argsort()[-5:][::-1]
    return df.iloc[top_indices]["label"].values

# Get Recommendations
print("LSI Recommendations:", lsi_recommendation())
